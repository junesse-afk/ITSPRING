 ...
 * 	  Scheduler는 구현 과정이 간편하므로 단순 스케쥴링에 많이 사용됨	
 * 
 * [ Scheduler 구현 방법 ]
 * 1. root-context.xml 또는 servlet-context.xml 등 필요한 위치에 Scheduler 설정
 * 2. 스케쥴링으로 작업을 수행할 메서드 상단에 @Scheduled 어노테이션 지정하여 스케쥴러 동작
 ...
 * 	 ex) @Scheduled(fixedRate = 1000): 1초 간격으로 스케쥴링
 * 
...
//	@Scheduled(cron = "0 * * * * *") // 매일 매시 매분 0초 마다 작업이 실행됨
